...
# 1. TRIGGER BACKUP
if [ -f "$BACKUP_SCRIPT" ]; then
    echo "📦 Running Safety Backup..."
    bash "$BACKUP_SCRIPT"
else
    echo "⚠️  No backup script found. Skipping backup."
    sleep 2
fi
...
if [ -f "$STOP_SCRIPT" ]; then
    ...
    echo "🛑 Stop script missing. Sending manual stop..."
    screen -S "$SCREEN_NAME" -p 0 -X stuff "stop^M"
    sleep 10
fi
...
draw_ui() {
    ...
    print_line 12 "   \${GREEN}[1]\${NORM} ▶ Start Server      \${YELLOW}[6]\${NORM} 🔄 Update Server"
    ...
    print_line 17 "   \${RED}[K]\${NORM} ❌ Forcekill        \${YELLOW}[M]\${NORM} 📦 Install Modpack"
    print_line 18 "   \${BLUE}[U]\${NORM} 🔄 Update Tools     \${RED}[R]\${NORM} ⏪ Restore Backup"
